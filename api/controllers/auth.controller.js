...
export const google = async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            const {password: pass, ...rest} = user._doc; //remove passwrd from the user variable
            res.cookie('access_token', token, {httpOnly: true}).status(200).json(rest);
        } else {
            const generatedPassword = Math.random().toString(36).slice(-8) +  Math.random().toString(36).slice(-8);
            const hashedPassword = bcryptjs.hashSync(generatedPassword, 10); //hashsync makes the system wait for the hash to finish.
            ...
            const {password: pass, ...rest} = newUser._doc; //remove password from the newUser variable
            res.cookie('access_token', token, {httpOnly: true}).status(200).json(rest);
        }
    } catch (error) {
        next(error)
